...
gsm_session_save_set_session_id (GsmSessionSave *save, const char *id)
{
        ...
        save->file_path = get_save_path (id);
}
...
flush_to_disk (GsmSessionSave *save)
{
        ...
        if (g_log_get_debug_enabled ())
                g_debug ("GsmSessionSave: Flushing to disk: %s",
...
deserialize_app (GVariantIter *iter, gboolean dirty, SavedApp **out_app)
{
        ...
        while (g_variant_iter_loop (instances, "@a{sv}", &instance_variant)) {
                ...
                instance = saved_app_instance_new (app, instance_id);
                ...
                GError         **error)
{
        ...
        save->fallback_instance_id = g_steal_pointer (&fallback_instance_id);
...
gsm_session_save_constructed (GObject *object)
{
        ...
        state_dir = get_save_path (NULL);
        if (g_mkdir_with_parents (state_dir, 0755) < 0)
                                g_warning ("GsmSessionSave: Failed to ensure '%s': %m",
                                           state_dir);
...
launch_app (SavedAppInstance *instance)
{
        ...
        desktop_id = g_strconcat (instance->app->app_id, ".desktop", NULL);
