...
void gsm_generate_session_list(GtkWidget *sl);
...
gsm_window_new(void)
{
  GtkWidget *retval, *vbox, *hbox, *btnbar, *wtmp,
    *btn_load, *btn_close;
  ...
  P(hbox, list_sessions = gtk_list_new());
...
void
gsm_generate_session_list(GtkWidget *sl)
{
  ...
  g_return_if_fail(GTK_IS_LIST(sl));

  gtk_list_clear_items(GTK_LIST(sl), 0, -1);
  ...
  while((entry = readdir(smdir)) != NULL) {
    if(!strncmp(entry->d_name, SESSIONFILE_PREFIX,
      ...
      gtk_object_set_user_data(GTK_OBJECT(wtmp), asession_fn);
      lis = g_list_append(lis, wtmp);
      nents++;
    }
  }
  if(lis != NULL)
    gtk_list_append_items(GTK_LIST(sl), lis);
}
