...
gsm_system_null_init_iface (GsmSystemInterface *iface)
{
        ...
        iface->can_restart_to_firmware = (void *) return_false;
        iface->set_restart_to_firmware = (void *) do_nothing;
...
gboolean
gsm_system_can_restart_to_firmware (GsmSystem *system)
{
        return GSM_SYSTEM_GET_IFACE (system)->can_restart_to_firmware (system);
}

void
gsm_system_set_restart_to_firmware (GsmSystem *system,
                                    gboolean   enable)
{
        GSM_SYSTEM_GET_IFACE (system)->set_restart_to_firmware (system, enable);
}
