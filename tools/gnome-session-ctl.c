...
do_monitor_leader (void)
{
        ...
        data.fifo_fd = g_open (fifo_name, O_RDONLY, 0600);
        if (data.fifo_fd >= 0) {
                sd_notify (0, "STATUS=Watching session leader");
                g_unix_fd_add (data.fifo_fd, G_IO_HUP | G_IO_IN, leader_fifo_io_cb, &data);
        } else {
                /* This is pretty bad, as we can't monitor the leader.
                 * We can still run though and allow the session to
                 * live, after all, this is "just" a safeguard to make
                 * sure the session is always cleaned up. */
                sd_notify (0, "STATUS=WARNING: Cannot watch session leader");
                g_warning ("Could not open monitor fifo, session cleanup might not be reliable!");
        }
